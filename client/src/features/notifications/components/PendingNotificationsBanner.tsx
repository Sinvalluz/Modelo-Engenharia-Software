		...
		<section className='p-4 pb-0 sm:p-5 sm:pb-0'>
			{isExpanded ? (
					...
					<div className='flex items-start gap-3'>
						...
						<div className='min-w-0 flex-1 space-y-2'>
							<div className='flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between'>
								<div>
									<h2 className='font-semibold'>Notificações pendentes</h2>
									<p className='text-secondary-foreground'>
										{notifications.length === 1
											? 'Você tem 1 lembrete para registrar suas movimentações.'
											: `Você tem ${notifications.length} lembretes para registrar suas movimentações.`}
									</p>
								</div>
								<Button
									type='button'
									variant='outline'
									className='h-8 cursor-pointer self-start'
				...
				<Button
					type='button'
					variant='outline'
					className='h-10 cursor-pointer gap-2 border-[#2fae8f]/30 bg-[#2fae8f]/10 text-foreground hover:bg-[#2fae8f]/15'
					onClick={() => setIsExpanded(true)}
				>
					<BellRing className='size-4 text-[#1f7a6b]' />
					<span>
						{notifications.length === 1 ? '1 notificação pendente' : `${notifications.length} notificações pendentes`}
					</span>
