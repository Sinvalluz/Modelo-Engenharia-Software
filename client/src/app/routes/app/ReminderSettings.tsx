...
function formatNextRun(value: string | null | undefined) {
  ...
  if (Number.isNaN(parsedDate.getTime())) {
    ...
    if (Number.isNaN(fallbackDate.getTime())) return "Sem próximo envio";
    ...
    <div className="p-4 sm:p-6 flex flex-col flex-1 min-w-0 space-y-4">
      <div>
        ...
        <p className="text-sm text-secondary-foreground">
          Configure quando o Siscodep deve lembrar você.
        </p>
      </div>
          ...
          {reminderConfigQuery.isLoading ? (
            ...
            <form className="space-y-5" onSubmit={handleSubmit}>
              <div className="flex items-center justify-between gap-4 rounded-lg border p-3">
                <div>
                  ...
                  <p className="mt-1 text-sm text-secondary-foreground">
                    Quando ativo, o sistema gera uma notificação e envia um
                    e-mail no horário definido.
                  </p>
                </div>
                <Switch
                  id="reminder-active"
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-2">
                  ...
                  <Select
                    value={frequency}
                    onValueChange={(value) =>
                      setFrequency(value as ReminderFrequency)
                    }
                  >
                    <SelectTrigger
              ...
              <div className="rounded-lg bg-muted/40 p-3 text-sm text-secondary-foreground">
                Proximo envio:{" "}
                {formatNextRun(reminderConfigQuery.data?.data.nextRunAt)}
              </div>
